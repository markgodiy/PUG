function Get-Secrets {
    ...
    Name: Get-Secrets
    Author: Mark Go
    Purpose: Simple Password Manager   

    .EXAMPLE
    Get-Secrets

    SystemName IPaddress UserName Password
    ---------- --------- -------- --------
    system1    123.456.789.1 usr1     01000000d08c9...
    ...
    *Password displayed is encrypted
    ...
    SystemName IPaddress UserName Password
    ---------- --------- -------- --------
    system1    123.456.789.1 usr1     p@ss

    *Password is decrypted and displayed in plaintext

    .EXAMPLE
    Get-Secrets -GUI

    Use -GUI switch to display GUI version
    #>

    param (
        [string]$SecretFile = "$env:APPDATA\Secrets.json",
        [switch]$GUI,
        [switch]$Decrypted = $false
    )
    ...
    # Hiding until I can figure out how to do this properly
    ...
    # Note by the author, for the Author: Add Edit button again once you figure out how to enable it properly
    ...
    function Display_SecretsTable {
        ...
        if ($existingData) {
            foreach ($item in $existingData) {
                ...
                if ($Decrypted) {
                    $pw = DecryptSecret($($item.Password))
                }
                else {
                    $pw = $item.Password
                }

                $objSecret = [PSCustomObject]@{
                    SystemName = $item.SystemName
                    IPaddress  = $item.IPAddress
                    UserName   = $item.UserName
                    Password   = $pw
                }
                $arrSecrets += $objSecret
            }
        }
        $arrSecrets
    }
    ...
    Function Save_Secrets($newSecret) {
        ...
        $existingData | ConvertTo-Json | Out-File -FilePath $SecretFile -Encoding UTF8
    }
    ...
    if ($GUI) {
        ...
        Display_SecretsTable
    }


} # End Function Get-Secrets
